...
function PRD() {
  const handleClick = () => {
    return (
      <Link variant="outlined" style={{ textDecoration: 'none', color: "#111111" }} />
    );
  }
    ;
  return (
    <>
      <Card sx={{ maxWidth: 345 }}>
        ...
        <Box marginTop={5} sx={{ border: 1, color: '#e6e6e6' }}>
          ...
          <Stack sx={{ textAlign: 'center', justifyContent: 'center' }} direction="row" spacing={1} marginTop={3}>

            <CardActions sx={{ border: 1 }} onClick={handleClick}>
              {/* <Link variant="outlined" style={{ textDecoration: 'none', color: "#111111" }} > */}

              <iconify-icon icon="ic:baseline-reorder" color='000000' />
              <Typography sx={{ color: '#111111' }}>
                Đặt mua
              </Typography>
            </CardActions>
            <CardActions sx={{ border: 1 }}>
              <iconify-icon icon="mdi:eye" color={'#111111'} />
            </CardActions>
